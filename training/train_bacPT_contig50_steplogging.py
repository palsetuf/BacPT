...
def train_bacPT(args,ratio, mask_prob, fixed_mask, batch_size, save_dir):
    ...
    if accelerator.is_main_process:
        ...
        print('~~~', seed_value)
    full_dataset = BLM.ContigGenomeDataset(num_samples=args.num_samples_total, max_seq_len = args.max_seq_len,\
    ...
    if accelerator.is_main_process and not args.no_eval and not args.continue_training:
        evaluate_bacPT(full_dataset.ipca.mean_,full_dataset.ipca.var_,-1,writer,model, test_dataloader, args.mask_prob, args.batch_size, accelerator, args.fixed_mask)
    ...
    for epoch in range(begin_epoch, begin_epoch + epochs):
        ...
        if epoch%args.eval_freq == 0 and accelerator.is_main_process and not args.no_eval:
            evaluate_bacPT(full_dataset.ipca.mean_,full_dataset.ipca.var_,epoch,writer,model, test_dataloader, args.mask_prob, args.batch_size, accelerator, args.fixed_mask)
...
def main(args):
    train_bacPT(args,ratio = args.ratio_samples_train, mask_prob = args.mask_prob, fixed_mask = args.fixed_mask,\
...
if __name__ == "__main__":
    ...
    parser.add_argument('--id-vs-num-filepath', type=pathlib.Path, required=True, help='CSV mapping genome IDs to their number of 50-protein contigs (Genome_ID, num_50l_contigs columns), e.g. the paper\'s train_proteins_with_contig_len.csv.')
